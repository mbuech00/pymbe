...
def init_mol(molecule):
		...
		with open('input-mol.inp') as f:
			# load file into content list
			content = f.readlines()
			# read in zmat/coordinates
			for i in range(0,len(content)-4):
				if (i == 0):
					molecule['mol'] = str(content[i])
				else:
					molecule['mol'] += str(content[i])
			# read in remaining mol info
			for j in range(1,5):
		...
		# verify that all necessary input is present
...
def init_backend_prog(molecule):
		""" set backend program for correlated calcs """
		# cfour
		if (molecule['backend_prog'] == 'CFOUR'):
			# is cfour bins in PATH?
...
def sanity_chk(molecule):
		""" various sanity checks for the input parameters """
		# type of expansion
		if ((molecule['exp'] != 'occ') and (molecule['exp'] != 'virt') \
				and (molecule['exp'] != 'comb-ov') and (molecule['exp'] != 'comb-vo')):
			molecule['error_msg'] = 'wrong input -- valid choices for expansion scheme are occ, virt, comb-ov, or comb-vo'
			molecule['error_code'] = 0
			molecule['error'].append(True)
		# expansion model
		if (not ((molecule['model'] == 'FCI') or (molecule['model'] == 'MP2') \
				or (molecule['model'] == 'CISD') or (molecule['model'] == 'CCSD') or (molecule['model'] == 'CCSDT'))):
		...
		if ((molecule['exp'] == 'occ') or (molecule['exp'] == 'virt')):
			if (molecule['prim_exp_thres'] < 0.0):
				...
				molecule['error_code'] = 0
				molecule['error'].append(True)
			if (molecule['prim_exp_scaling'] < 1.0):
				molecule['error_msg'] = 'wrong input -- expansion scaling (prim_exp_scaling) must be float > 1.0'
				molecule['error_code'] = 0
				molecule['error'].append(True)
