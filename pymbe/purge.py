...
from pymbe.tools import inc_dim, inc_shape, min_orbs, tuples, hash_1d, hash_lookup
...
def main(
    mpi: MPICls, exp: ExpCls
) -> Tuple[List[MPI.Win], List[MPI.Win], Dict[str, List[int]]]:
    """
    ...
    if (
        exp.order == exp.min_order
        or exp.screen_orbs.size == 0
        or exp.exp_space[-1].size < exp.order + 1
    ):
        exp.time["purge"].append(0.0)
    ...
    for k in range(exp.min_order, exp.order + 1):
        ...
        hashes = np.ndarray(
            ...
            dtype=np.int64,
            shape=(exp.n_tuples["inc"][k - exp.min_order],),
        )
        ...
        inc = np.ndarray(
            buffer=buf,  # type: ignore
            dtype=np.float64,
            shape=inc_shape(exp.n_tuples["inc"][k - exp.min_order], dim),
        )
        ...
        hashes_win = MPI.Win.Allocate_shared(
            8 * np.sum(recv_counts) if mpi.local_master else 0,
            8,
