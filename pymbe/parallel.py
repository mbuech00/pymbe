...
def mpi_reduce_screen_dict(
    ...
    """
    this function performs a reduce operation on the numpy arrays in screening
    dictionaries
    """
    keys = list(set(flatten_list(comm.allgather(list(screen_dict)))))
    for key in keys:
        try:
            screen_dict[key] = (
                mpi_reduce(comm, screen_dict[key][0], root=0, op=MPI.SUM),
                mpi_reduce(comm, screen_dict[key][1], root=0, op=MPI.SUM),
                mpi_reduce(comm, screen_dict[key][2], root=0, op=MPI.SUM),
                mpi_reduce(comm, screen_dict[key][3], root=0, op=MPI.SUM),
            )
        except KeyError:
            screen_dict[key] = (
                mpi_reduce(comm, np.zeros(length, dtype=np.int64), root=0, op=MPI.SUM),
                mpi_reduce(
                    comm, np.zeros(length, dtype=np.float64), root=0, op=MPI.SUM
                ),
                mpi_reduce(
                    comm, np.zeros(length, dtype=np.float64), root=0, op=MPI.SUM
                ),
