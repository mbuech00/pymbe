...
test_cases_main = [
    (
        ...
        np.array([-0.0374123708341898]),
        np.array([-0.004676546354273725]),
        np.array([0.0018267714680604286]),
        np.array([0.010886635891736773]),
    ),
    (
        "h2o",
        2,
        ...
        np.array([-0.11605435599270209]),
        np.array([-0.004144798428310789]),
        np.array([0.0001698239845069338]),
        np.array([0.009556269221292268]),
    ),
]
...
test_cases_sum = [
    ...
    (
        "dipole",
        np.array([1, 7, 8], dtype=np.int64),
        np.array([1.21776657, 1.21776657, 1.21776657], dtype=np.float64),
    ),
    (
        "excitation",
        np.array([1, 7, 8], dtype=np.int64),
    ...
    (
        "energy",
        np.array([1, 7, 8, 9], dtype=np.int64),
        2.7229882355444195,
    ),
    (
        "dipole",
        np.array([1, 7, 8, 9], dtype=np.int64),
        np.array([2.72298824, 2.72298824, 2.72298824], dtype=np.float64),
    ),
    (
        "excitation",
        np.array([1, 7, 8, 9], dtype=np.int64),
...
@pytest.mark.parametrize(
    ...
    "ref_inc_amax, ref_tot, ref_mean_inc, ref_min_inc, ref_max_inc",
...
@pytest.mark.parametrize(argnames="system", argvalues=["hubbard"], indirect=True)
def test_inc(ints: Tuple[np.ndarray, np.ndarray]) -> None:
    """
    this function tests _inc
    """
    n = 6
    occup = np.array([2.0] * (n // 2) + [0.0] * (n // 2), dtype=np.float64)
...
@pytest.mark.parametrize(
    argnames="target, tup, ref_res",
    argvalues=test_cases_sum,
    ids=[case[0] + "-" + str(case[1]) for case in test_cases_sum],
)
def test_sum(target: str, tup: np.ndarray, ref_res: Union[float, np.ndarray]) -> None:
    """
    ...
    res = _sum(nocc, target, min_order, tup.size, inc, hashes, min_occ, min_virt, tup)
