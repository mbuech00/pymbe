...
def main(
    ...
    eri = np.ndarray(
        ...
        dtype=np.float64,
        shape=(exp.norb * (exp.norb + 1) // 2,) * 2,
    )

    # load hcore
    buf = exp.hcore.Shared_query(0)[0]
    hcore = np.ndarray(
        buffer=buf, dtype=np.float64, shape=(exp.norb,) * 2  # type: ignore
    )
    ...
    vhf = np.ndarray(
        buffer=buf,  # type: ignore
        dtype=np.float64,
        shape=(exp.nocc, exp.norb, exp.norb),
    )
    ...
    if rst_read:
        ...
        hashes_win = MPI.Win.Allocate_shared(
            8 * exp.n_tuples["inc"][-1] if mpi.local_master else 0,
            8,
    ...
    hashes.append(
        np.ndarray(
            buffer=buf, dtype=np.int64, shape=(exp.n_tuples["inc"][-1],)  # type: ignore
        )
    )
    ...
    for k in range(exp.order - exp.min_order):
        ...
        inc.append(
            np.ndarray(
                buffer=buf,  # type: ignore
                dtype=np.float64,
                shape=inc_shape(exp.n_tuples["inc"][k], dim),
            )
        )
    ...
    if rst_read:
        ...
        inc_win = MPI.Win.Allocate_shared(
            8 * exp.n_tuples["inc"][-1] * dim if mpi.local_master else 0,
            8,
            comm=mpi.local_comm,  # type: ignore
        )
    ...
    if exp.ref_space.size == 0 and exp.order == 2 and exp.base_method is None:
        pair_corr: Optional[List[np.ndarray]] = [
            np.zeros(exp.n_tuples["inc"][exp.order - exp.min_order], dtype=np.float64),
            np.zeros(
                [exp.n_tuples["inc"][exp.order - exp.min_order], 2], dtype=np.int32
            ),
        ]
    else:
        pair_corr = None
    ...
    for tup_idx, tup in enumerate(
        ...
        if rst_write and tup_idx % exp.rst_freq < mpi.global_size:
            ...
            # update rst_write
            rst_write = (
                mbe_idx + exp.rst_freq < exp.n_tuples["inc"][-1] - mpi.global_size
            )
            ...
            if mpi.global_master:
                ...
                time = MPI.Wtime()
                # print status
                logger.info(mbe_status(exp.order, mbe_idx / exp.n_tuples["inc"][-1]))
    ...
    # print final status
    if mpi.global_master:
        logger.info(mbe_status(exp.order, 1.0))
    ...
    thres = 1.0 if exp.order < exp.screen_start else exp.screen_perc
    screen_idx = int(thres * exp.exp_space[-1].size)
    exp.exp_space.append(
        exp.exp_space[-1][np.sort(np.argsort(nonzero_screen)[::-1][:screen_idx])]
    )
    ...
    if mpi.global_master and pair_corr is not None:
        ...
        logger.debug(" " + "-" * 74)
        logger.debug(
            " orbital tuple  |  absolute corr.  |  relative corr.  |  cumulative corr."
        )
        logger.debug(" " + "-" * 74)
        for i in range(10):
            logger.debug(
