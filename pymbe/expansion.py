...
def int_wins(
    ...
    hcore_win = MPI.Win.Allocate_shared(
        8 * norb**2 if mpi.local_master else 0, 8, comm=mpi.local_comm  # type: ignore
    )
...
def restart_main(mpi: MPICls, exp: ExpCls) -> int:
    ...
    for i in range(len(files)):
        ...
        if "mbe_hashes" in files[i]:
            ...
            hashes = np.ndarray(
                buffer=buf, dtype=np.int64, shape=(n_tuples,)  # type: ignore
            )
            if mpi.global_master:
        ...
        # read increments
            ...
            exp.incs.append(
                MPI.Win.Allocate_shared(
                    8 * n_tuples * inc_dim(exp.target) if mpi.local_master else 0,
                    8,
                    comm=mpi.local_comm,  # type: ignore
                )
            )
            ...
            inc = np.ndarray(
                buffer=buf,  # type: ignore
                dtype=np.float64,
                shape=inc_shape(n_tuples, inc_dim(exp.target)),
            )
            if mpi.global_master:
