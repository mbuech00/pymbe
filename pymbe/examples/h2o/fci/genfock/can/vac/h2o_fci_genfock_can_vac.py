...
from pyscf import gto, scf, symm, ao2mo, mcscf
...
def mbe_example(rst=True):
    ...
    if MPI.COMM_WORLD.Get_rank() == 0 and not os.path.isdir(os.getcwd() + "/rst"):
        ...
        exact_gen_fock = np.empty((nocc + ncas, norb), dtype=np.float64)

        # calculate occupied-general generalized Fock matrix elements
        exact_gen_fock[occ_idx] = 2 * (inact_fock[:, occ_idx] + act_fock_pi).transpose()

        # calculate active-general generalized Fock matrix elements
        exact_gen_fock[cas_idx] = np.einsum(
            "uv,pv->up", rdm1, inact_fock[:, cas_idx]
        ) + np.einsum("uvxy,pvxy->up", rdm2, eri_gaaa)
