...
from pymbe import MBE, MPICls, mpi_finalize, hf, base, ints, ref_prop
...
def mbe_example() -> MBE:
    ...
    if mpi.global_master and not os.path.isdir(os.getcwd()+'/rst'):
        ...
        # base model
        base_energy = base('ccsd', mol, hf_object, mo_coeff, occup, orbsym, \
                           norb, ncore, nocc)
        ...
        ref_energy = ref_prop(mol, hcore, vhf, eri, occup, orbsym, nocc, \
                              ref_space, base_method='ccsd', hf_prop=hf_energy)
        ...
        mbe = MBE(method='fci', mol=mol, ncore=ncore, nocc=nocc, norb=norb, \
                  ...
                  base_method='ccsd', base_prop=base_energy)
