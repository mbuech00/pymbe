...
from pymbe import MBE, MPICls, mpi_finalize, hf, ints, dipole_ints, ref_prop
...
def mbe_example() -> MBE:
    ...
    if mpi.global_master and not os.path.isdir(os.getcwd()+'/rst'):
        ...
        ref_dipole = ref_prop(mol, hcore, vhf, eri, occup, orbsym, nocc, \
                              ref_space, method='ccsd', target='dipole', \
                              hf_prop=hf_dipole, dipole_ints=dip_ints)

        # create mbe object
        mbe = MBE(method='ccsd', target='dipole', mol=mol, ncore=1, nocc=nocc, \
