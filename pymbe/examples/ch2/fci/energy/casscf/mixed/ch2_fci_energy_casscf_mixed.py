...
def mbe_example(rst=True) -> Optional[Union[float, np.ndarray]]:
    ...
    if MPI.COMM_WORLD.Get_rank() == 0 and not os.path.isdir(os.getcwd()+'/rst'):
        ...
        # casscf orbitals
        mo_coeff, orbsym = ref_mo('casscf', mol, hf_object, mo_coeff, occup, \
                                  orbsym, norb, ncore, nocc, nvirt, ref_space, \
                                  fci_solver='pyscf_spin1', wfnsym=['b2'], \
                                  weights=[1.])
        ...
        ref_energy = ref_prop(mol, hcore, vhf, eri, occup, orbsym, nocc, \
                              ref_space, fci_solver='pyscf_spin1', \
                              fci_state_sym='b2', hf_prop=hf_energy, \
                              orb_type='casscf')
        ...
        mbe = MBE(method='fci', fci_solver='pyscf_spin1', mol=mol, \
                  ...
                  orb_type='casscf', hcore=hcore, vhf=vhf, eri=eri, \
                  ref_space=ref_space, ref_prop=ref_energy, rst=rst)
