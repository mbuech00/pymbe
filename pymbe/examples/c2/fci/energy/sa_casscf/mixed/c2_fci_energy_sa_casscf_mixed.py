...
def mbe_example(rst=True):
    ...
    if MPI.COMM_WORLD.Get_rank() == 0 and not os.path.isdir(os.getcwd() + "/rst"):
        ...
        # wavefunction symmetries and weights for for state-averaged casscf orbitals
        wfnsym = ["ag", "ag", "ag", "b1g"]
        ...
        cas = mcscf.CASSCF(hf, ref_space.size, act_nelec).set(
            conv_tol=1.0e-10, max_cycle_macro=500, frozen=ncore
        )

        # create unique list of wfnsym while maintaining order
        unique_wfnsym = list(dict.fromkeys(wfnsym))

        # initialize list of fcisolvers
        fcisolvers = []

        # loop over symmetries and add fcisolvers to list
        for i in range(0, len(unique_wfnsym)):
            fcisolvers.append(
                fci.direct_spin0_symm.FCI(mol).set(
                    conv_tol=1.0e-10,
                    orbsym=orbsym_cas[ref_space],
                    wfnsym=unique_wfnsym[i],
                    nroots=np.count_nonzero(np.asarray(wfnsym) == unique_wfnsym[i]),
                )
            )
