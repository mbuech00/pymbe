...
def main(mpi: parallel.MPICls, mol: system.MolCls, calc: calculation.CalcCls, \
        ...
        for k in range(exp.order-1, exp.min_order-1, -1):
            ...
            hashes = np.ndarray(buffer=buf, dtype=np.int64, shape=(exp.n_tuples[k-exp.min_order],))

            buf = exp.prop[calc.target_mbe]['inc'][k-exp.min_order].Shared_query(0)[0] # type: ignore
            inc = np.ndarray(buffer=buf, dtype=np.float64, shape=shape(exp.n_tuples[k-exp.min_order], dim))
            ...
            # update n_tuples
            exp.n_tuples[k-exp.min_order] = int(np.sum(recv_counts))
            if mpi.global_master:
                print('k = {:} - n_tuples = {:}'.format(k, exp.n_tuples[k-exp.min_order]))
