...
from tools import time_str, fsum, hash_2d, hash_1d, hash_lookup, tuples, \
                  start_idx, _comb_idx, _idx, n_tuples, cas, core_cas, \
                  _cas_idx_cart, _coor_to_idx, idx_tril, pi_space, _pi_orbs, \
                  pi_prune, occ_prune, virt_prune, nelec, ndets, mat_idx, \
...
def test_fsum(a: np.ndarray, ref_sum: float):
        """
...
def test_hash_lookup(b: np.ndarray, present: bool):
        """
...
def test_tuples(ref_space: np.ndarray, ref_n_tuples: int):
        """
...
def test_comb_idx(tup: np.ndarray, ref_idx: float):
        """
...
def test_idx(order: int, ref_idx: float):
        """
...
def test_n_tuples(ref_occ: bool, ref_virt: bool, ref_n_tuples: int):
        """
...
def test_pi_prune(tup: np.ndarray, ref_bool: bool):
        """
...
def test_occ_prune(tup: np.ndarray, ref_bool: bool):
        """
...
def test_virt_prune(tup: np.ndarray, ref_bool: bool):
        """
...
def test_nelec(tup: np.ndarray, ref_nelec: Tuple[int, int]):
        """
...
def test_ndets(cas_idx: np.ndarray, ref_space: np.ndarray, n_elec: Tuple[int, int], ref_ndets: int):
        """
...
def test_mat_idx(site_idx: int, nx: int, ny: int, ref_idx_tup: Tuple[int, int]):
        """
...
def test_near_nbrs(site_xy: Tuple[int, int], nx: int, ny: int, ref_nbrs: List[Tuple[int, int]]):
        """
...
def test_natural_keys(test_string: str, ref_keys: List[Union[int, str]]):
        """
...
def test_convert(test_string: str, ref_type: type):
        """
...
def test_inc_dim(target: str, ref_dim: int):
        """
...
def test_inc_shape(n: int, dim: int, ref_shape: Union[Tuple[int], Tuple[int, int]]):
        """
        this function tests inc_dim
        """
