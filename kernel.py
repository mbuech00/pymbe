...
class KernCls():
		...
		def hf(self, mol, calc):
				...
				mol.norb, mol.nocc, mol.nvirt = self.dim(hf, mol.ncore, calc.exp_type)
		...
		def calc(self, mol, calc, exp, method):
				...
				# fci calc
				if (method == 'FCI'):
					e_corr = self.fci(mol, calc, exp, calc.mo, False)
				# sci base
				elif (method == 'SCI'):
					e_corr, _ = self.sci(mol, calc, exp, calc.mo, False)
				# cisd calc
				elif (method == 'CISD'):
					e_corr, _ = self.ci(mol, calc, exp, calc.mo, False)
				# ccsd / ccsd(t) calc
				elif (method in ['CCSD','CCSD(T)']):
					e_corr, _ = self.cc(mol, calc, exp, calc.mo, False, (method == 'CCSD(T)'))
				#
				return e_corr


		def base(self, mol, calc, exp):
				""" calculate base energy and mo coefficients """
				# set core and cas spaces
				exp.core_idx, exp.cas_idx = self.core_cas(mol, exp, calc.exp_space)
				# zeroth-order energy
				if (calc.exp_base['METHOD'] is None):
					e_base = 0.0
				# cisd base
				elif (calc.exp_base['METHOD'] == 'CISD'):
					e_base, dm = self.ci(mol, calc, exp, calc.mo, True)
					if ((mol.spin > 0) and (dm is not None)): dm = dm[0] + dm[1]
				# ccsd / ccsd(t) base
				elif (calc.exp_base['METHOD'] in ['CCSD','CCSD(T)']):
					e_base, dm = self.cc(mol, calc, exp, calc.mo, True, \
												(calc.exp_base['METHOD'] == 'CCSD(T)') and \
												((calc.exp_occ == 'REF') and (calc.exp_virt == 'REF')))
					if ((mol.spin > 0) and (dm is not None)): dm = dm[0] + dm[1]
				# sci base
				elif (calc.exp_base['METHOD'] == 'SCI'):
					e_base, dm = self.sci(mol, calc, exp, calc.mo, True)
				# copy mo
				mo = np.copy(calc.mo)
				...
				if ((calc.exp_occ == 'NO') or (calc.exp_virt == 'NO')):
					if (calc.exp_base['METHOD'] == 'CCSD(T)'):
						e_base, dm = self.cc(mol, calc, exp, mo, False, True)
					elif (calc.exp_base['METHOD'] == 'SCI'):
						e_base, dm = self.sci(mol, calc, exp, mo, False)
				#
				return e_base, mo


		def casscf(self, mol, calc, exp, method):
				""" casscf calc """
				# casscf ref
