...
def init_mpi_timings(molecule):
		...
		molecule['store_time'] = 0.0; molecule['store_key'] = ''
		# mpi distribution
		if (molecule['rst']):
...
def calc_mpi_timings(molecule):
		...
		if (not molecule['mpi_parallel']):
			...
			for i in range(0,3):
				...
				for j in range(0,molecule['mpi_size']):
					dist[0][j] = (molecule['sum_work_abs'][i][j]/(molecule['sum_work_abs'][i][j] \
									+molecule['sum_comm_abs'][i][j]+molecule['sum_idle_abs'][i][j]))*100.0
					dist[1][j] = (molecule['sum_comm_abs'][i][j]/(molecule['sum_work_abs'][i][j] \
									+molecule['sum_comm_abs'][i][j]+molecule['sum_idle_abs'][i][j]))*100.0
					dist[2][j] = (molecule['sum_idle_abs'][i][j]/(molecule['sum_work_abs'][i][j] \
									+molecule['sum_comm_abs'][i][j]+molecule['sum_idle_abs'][i][j]))*100.0
